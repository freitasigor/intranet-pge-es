...
; (function ($, window, document, undefined) {
    ...
    Filtrify.prototype.set = function () {
        var f = 0, field,
			browser = $.browser;
        this._menu.list = $("<ul class='ft-menu row' />");
        ...
        $(".filter-licitacao").show();
    };

    Filtrify.prototype.build = function (f) {
        var html, t, tag, tags = [];

        html = "<li class='ft-field col-xs-12 col-sm-6 col-md-4 col-lg-4'>" +
        "<label>" + f + "</label>" +
		"<span class='ft-label'>Selecione</span>" +
        ...
        tags.sort();
        /*
        var totalItens = this._items
            .filter(function () {
                return ($.inArray("item-link", this.classList)) > -1;
            }).length;
        */
        html += "<li class=\"all\" >Todos</li>"; 
        for (t = 0; t < tags.length; t++) {
            tag = tags[t];
    ...
    Filtrify.prototype.events = function (f) {
        ...
        this._menu[f].tags.on("click", "li", this._bind(function () {
            ...
            //console.log(this._menu[f].selected.text())
            for (i = (this._order.indexOf(f) + 1) ; i < this._order.length; i++) {
                this._query[this._order[i]] = [];
                this._menu[this._order[i]].label.text("Todos");
            }
            if (this._menu[f].selected.text() != "") {                
                this.unselectCustom(f, this._menu[f].selected.text());
                //this.removeFromSelected(f, this._menu[f].selected.text());
            }
            ...
            //console.log("Query: " + JSON.stringify(this._query));
        }, this));
    ...
    Filtrify.prototype.openPanel = function (f) {
        ...
        for (key in this._menu) {
            if (key != "list" && key != f) {
                ...
                this._menu[key].panel.addClass("ft-hidden");
                this._menu[key].label.removeClass("ft-opened");
                //console.log(key + ": " + this._menu[key]);
            }
        }
    ...
    Filtrify.prototype.selectSingle = function (f) {

        if (this._menu[f].highlight.text() != "Todos") {
            this.updateQueryTagsAdd(f, this._menu[f].highlight.text());
    ...
    Filtrify.prototype.updateQueryTagsAdd = function (f, tag) {
        var index = $.inArray(tag, this._query[f]);
    ...
    Filtrify.prototype.updateQueryTagsRemove = function (f, tag) {
        var index = $.inArray(tag, this._query[f]);
        if (index !== -1) {
            this._query[f].splice(index, 1);
        }
    ...
    Filtrify.prototype.appendToSelected = function (f) {
        ...
        var qtdItens = this._menu[f].label[0].childElementCount;

        if (this._menu[f].label[0].childElementCount == 0) {
            this._menu[f].label.text("");
            this._menu[f].label.append("<span>" + this._menu[f].highlight.context.innerText + "</span>");
        }
        else {
            this._menu[f].label[0].children[(qtdItens - 1)].className = "comma-after";
            this._menu[f].label.append("<span>"+this._menu[f].highlight.context.innerText + "</span>");
        }
    ...
    Filtrify.prototype.unselectCustom = function (f, tag) {

        this.updateQueryTagsRemove(f, tag);
        this.removeFromSelected(f, tag);
        this.removeFromActive(f, tag);
        this.updateActiveClass(f);
        this.resetSearch(f);
    ...
    Filtrify.prototype.removeFromSelected = function (f, tag) {
        ...
        if (this._menu[f].label[0].childElementCount == 1) {
            this._menu[f].label[0].children[0].className = "";
        }
        else if (this._menu[f].label[0].childElementCount == 0) {
            this._menu[f].label.text("Todos");
        }
    ...
    Filtrify.prototype.filterCustom = function (field) {
        var f, r, t, c, m;
        ...
        //this.rewriteFields();
        this.rewriteFieldsCustom(field);
        this.callback();
    ...
    Filtrify.prototype.rewriteFieldsCustom = function (fieldActive) {
        var field;
        var firstField = "";
        for (field in this._fields) {
            this._menu[field].tags
				...
				.each(this._bind(function (index, element) {
				    ...
				    if (field != firstField && field != fieldActive) {
				        element.setAttribute("data-count", count);
				    }
				}, this));
        };
